 ...
 *
 * Bridges useTFLData (React Query) with Zustand stores for the TFL Designer page. This pattern allows optimistic local
 * updates while maintaining server sync.
 */
import { useCallback, useEffect } from 'react';

import { useAnalysisScopeNodeId } from '@/features/clinical-context';
import { useFigureStore, useListingStore, useTableStore } from '@/features/tfl-designer';
...
export function useTFLDesignerData() {
  ...
  const saveCurrentFigure = useCallback(
    async (userId: string): Promise<boolean> => {
  ...
  const saveCurrentListing = useCallback(
    async (userId: string): Promise<boolean> => {
