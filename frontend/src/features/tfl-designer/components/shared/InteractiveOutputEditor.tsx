 ...
 *
 * Every logical block (title, population, header-rows, data-rows, footnotes, abbreviations) is independently selectable
 * and editable. Rows can reorder up/down; columns can reorder left/right.
 *
 * Modern UX: hover between rows/columns to get a blue "+" insert line. Hover a row to see move/delete/indent actions in
 * a context bar.
 */
import {
  AlignCenterOutlined,
...
import { Button, ColorPicker, Divider, Empty, Tooltip, message } from 'antd';
...
import { DEFAULT_DECIMAL_RULES, generateId } from '../../types';
import { buildDecimalsMap, formatPlaceholder } from '../../utils/placeholderFormatter';
import {
  addChildToTree,
  collectLeaves,
  countLeaves,
...
const Toolbar: React.FC<ToolbarProps> = ({ fmt, onClose, onFmt, pos, visible }) => {
  ...
  return (
    <div
      className="fixed z-50 flex items-center gap-1 border border-gray-200 rounded-lg bg-white px-2 py-1.5 shadow-xl"
      ref={ref}
    ...
    >
      ...
      {btn('Underline (Ctrl+U)', fmt.underline, <UnderlineOutlined />, () =>
        onFmt({ ...fmt, underline: !fmt.underline })
      )}
      <Divider
      ...
      <ColorPicker
        size="small"
        value={fmt.color || '#000'}
        onChange={c => onFmt({ ...fmt, color: c.toHexString() })}
      />
      <Divider
        style={{ height: 20, margin: '0 4px' }}
        type="vertical"
      />
      {btn('Align Left', fmt.align === 'left' || !fmt.align, <AlignLeftOutlined />, () =>
        onFmt({ ...fmt, align: 'left' })
      )}
...
/**
 * Apply formatting via document.execCommand on the currently focused contentEditable. This is the only reliable way to
 * make bold/italic/ underline/alignment take effect inside contentEditable (CSS inheritance alone doesn't override
 * browser-internal formatting nodes).
 */
...
const InlineEdit: React.FC<InlineEditProps> = ({
  ...
  const input = useCallback(
    (e: React.FormEvent<HTMLDivElement>) => onChange(e.currentTarget.textContent || ''),
    [onChange]
  );
  ...
  const key = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (k === 'b') {
        ...
        onFmt({ ...fmt, bold: !fmt.bold });
      } else if (k === 'i') {
        e.preventDefault();
        document.execCommand('italic');
        onFmt({ ...fmt, italic: !fmt.italic });
      } else if (k === 'u') {
        e.preventDefault();
...
const RowInsertGap: React.FC<{ onInsert: () => void; tooltip?: string }> = ({ onInsert, tooltip }) => (
  <tr className="row-gap">
    <td
      className="relative border-b-0 px-0 py-0"
      colSpan={99}
      style={{ height: 2, lineHeight: 0 }}
    >
      <div
        className="absolute inset-x-0 z-10 flex justify-center -top-[9px]"
        style={{ height: 20 }}
      >
        <Tooltip title={tooltip || 'Insert row here'}>
          <button
            className="h-5 w-5 flex cursor-pointer items-center justify-center border-0 rounded-full bg-blue-500 text-xs text-white leading-none opacity-0 shadow-sm transition-opacity hover:opacity-100"
...
const FootnoteInsertGap: React.FC<{ onInsert: () => void; tooltip?: string }> = ({ onInsert, tooltip }) => (
  <div
    className="group-fn-gap relative"
    style={{ height: 4 }}
  >
    <div
      className="absolute inset-x-0 z-10 flex justify-center -top-[9px]"
    ...
    >
      <Tooltip title={tooltip || 'Insert footnote here'}>
        <button
          className="group-fn-gap-hover:opacity-100 h-4 w-4 flex cursor-pointer items-center justify-center border-0 rounded-full bg-blue-500 text-white leading-none opacity-0 shadow-sm transition-opacity"
...
const MoveBtn: React.FC<{
  ...
  onNext: () => void;
  onPrev: () => void;
}> = ({ dir, onFirst, onLast, onNext, onPrev }) => {
  const Up =
    dir === 'v' ? (
  ...
  return (
    <span
      className={`inline-flex ${dir === 'h' ? 'flex-row' : 'flex-col'} opacity-0 group-hover:opacity-100 transition-opacity leading-none`}
    >
      <Button
  ...
  function InteractiveOutputEditor(
    ...
    React.useImperativeHandle(ref, () => ({
      ...
      undo: () => {
        ...
        redoStack.current.push(structuredClone(template!));
    ...
    useEffect(() => {
      ...
      const f: Record<string, TextFormat> = {
        population: { align: 'center', fontSize: compact ? 12 : 14 },
        title: { align: 'center', bold: true, fontSize: compact ? 12 : 14 }
      };
      (s.rows || []).forEach((r: TableRow) => {
        f[`row-${r.id}`] = { bold: r.level === 0, fontSize: 11 };
      });
      (s.columns || []).forEach((c: ListingColumn) => {
        f[`col-${c.id}`] = { bold: true, fontSize: 11 };
      });
      (s.footer?.notes || []).forEach((_: string, i: number) => {
        f[`fn-${i}`] = { fontSize: 9, italic: true };
      });
    ...
    const setShell = useCallback(
      (updater: (s: any) => void) => {
        ...
        const s = {
          ...template.shell,
          columns: [...((template.shell as any).columns || [])],
          footer: (template.shell as any).footer
            ? { ...(template.shell as any).footer, notes: [...((template.shell as any).footer?.notes || [])] }
            : undefined,
          rows: [...((template.shell as any).rows || [])]
        } as any;
        updater(s);
    ...
    const onOutputNumberChange = useCallback(
      (v: string) => {
        setShell(sh => {
    ...
    /* ---- row move / delete ---- */
    /**
     * Find the index range [start, end) occupied by a row and all its descendants. A row at `rows[idx]` with level L
     * "owns" all consecutive rows after it whose level > L.
     */
    ...
    const nextSibling = useCallback(
      (rows: TableRow[], idx: number): number => {
        const level = rows[idx].level || 0;
    ...
    const delRow = useCallback(
      (rid: string) => {
        setShell(sh => {
          sh.rows = (sh.rows || []).filter((r: TableRow) => r.id !== rid);
        });
        ...
        setShell(sh => {
          sh.rows = (sh.rows || []).map((r: TableRow) => {
            if (r.id === rowId) {
              ...
              return { ...r, indent: newLevel, level: newLevel };
            }
            return r;
          });
        });
    ...
    const delCol = useCallback(
      (cid: string) => {
        setShell(sh => {
          sh.columns = (sh.columns || []).filter((c: ListingColumn) => c.id !== cid);
        });
    ...
    const onFnChange = useCallback(
      (idx: number, v: string) => {
        setShell(sh => {
          if (sh.footer?.notes)
            sh.footer = { ...sh.footer, notes: sh.footer.notes.map((n: string, i: number) => (i === idx ? v : n)) };
        });
      },
      [setShell]
    );
    ...
    const addFootnote = useCallback(() => {
      setShell(sh => {
        if (!sh.footer) sh.footer = { notes: [], source: sh.dataset };
    ...
    const delFootnote = useCallback(
      (idx: number) => {
        setShell(sh => {
          if (sh.footer?.notes)
            sh.footer = { ...sh.footer, notes: sh.footer.notes.filter((_: string, i: number) => i !== idx) };
        });
    ...
    const insertTitleLine = useCallback(
      (atIndex: number) => {
        setShell(sh => {
    ...
    const updateTitleLine = useCallback(
      (lineId: string, text: string) => {
        setShell(sh => {
          if (!sh.extraTitleLines) return;
          sh.extraTitleLines = sh.extraTitleLines.map((l: { id: string; text: string }) =>
    ...
    const deleteTitleLine = useCallback(
      (lineId: string) => {
        setShell(sh => {
          if (!sh.extraTitleLines) return;
    ...
    const moveTitleLine = useCallback(
      (idx: number, delta: number) => {
        setShell(sh => {
          if (!sh.extraTitleLines) return;
          const lines = [...sh.extraTitleLines];
    ...
    const updateHeaderLabel = useCallback(
      (headerId: string, newLabel: string) => {
    ...
    const addHeaderColumn = useCallback(
      (afterId?: string) => {
        const newCol: ColumnHeaderGroup = { align: 'center', id: generateId('hcol'), label: 'New Col', width: 120 };
    ...
    const addHeaderChild = useCallback(
      (parentId: string) => {
    ...
    const moveHeaderColumn = useCallback(
    ...
    const updateLabelColumn = useCallback(
    ...
    const addLabelColumn = useCallback(
      (afterId?: string) => {
        setShell(sh => {
          // Seed from default when shell has never been edited
          const lbls: LabelColumn[] = [
            ...(sh.labelColumns || [{ id: 'lbl_0', label: sh.rowLabel || 'Row Label', width: 280 }])
          ];
    ...
    const deleteLabelColumn = useCallback(
      (colId: string) => {
        setShell(sh => {
    ...
    const moveLabelColumn = useCallback(
    ...
    const updateColumnHeaderLines = useCallback(
      (colId: string, updater: (lines: string[]) => string[]) => {
        setShell(sh => {
          sh.columns = (sh.columns || []).map((c: ListingColumn) => {
    ...
    const updateHeaderLine = useCallback(
      (colId: string, lineIdx: number, value: string) => {
    ...
    const updateColumnWidth = useCallback(
      (colId: string, width: number) => {
        setShell(sh => {
          sh.columns = (sh.columns || []).map((c: ListingColumn) => (c.id === colId ? { ...c, width } : c));
        });
      },
      [setShell]
    );
    ...
    const mk = (
      ...
      value: string,
      onChange: (v: string) => void,
      opts?: { className?: string; multiline?: boolean; placeholder?: string }
    ) => (
      <InlineEdit
        className={opts?.className}
        editable={editable}
        eid={eid}
        fmt={fmts[eid] || { fontSize: 11 }}
        multiline={opts?.multiline}
        placeholder={opts?.placeholder}
        value={value}
        onChange={onChange}
        onFmt={f => setFmts(p => ({ ...p, [eid]: f }))}
        onSelect={(s, p) => selH(eid, s, p)}
      />
    );
    ...
    const TitleInsertGap: React.FC<{ onInsert: () => void; tooltip?: string }> = ({ onInsert, tooltip }) => (
      <div
        className="group-tl-gap relative"
        style={{ height: 4 }}
      >
        <div
          className="absolute inset-x-0 z-10 flex justify-center -top-[9px]"
        ...
        >
          <Tooltip title={tooltip || 'Insert line here'}>
            <button
              className="group-tl-gap-hover:opacity-100 h-4 w-4 flex cursor-pointer items-center justify-center border-0 rounded-full bg-blue-500 text-white leading-none opacity-0 shadow-sm transition-opacity"
              style={{ fontSize: 12, fontWeight: 300 }}
              onMouseDown={e => e.stopPropagation()}
              onClick={e => {
                e.stopPropagation();
    ...
    const renderTitleBlock = () => (
      ...
      >
        ...
        <div
          className="mb-1"
          style={{ textAlign: hs?.alignment || 'center' }}
        >
          {editable ? (
            <>
              {/* Output Number editable separately from title */}
              {mk('outNum', outNum, onOutputNumberChange, { className: 'font-semibold', placeholder: 'X.X.X' })}
              {outNum && <span>&nbsp;&nbsp;</span>}
        ...
        {editable && (
          ...
          `}</style>
            {extraTitleLines.length === 0 ? (
              <TitleInsertGap
                tooltip="Add a line"
                onInsert={() => insertTitleLine(0)}
              />
            ) : (
              <>
                <TitleInsertGap
                  tooltip="Insert line at top"
                  onInsert={() => insertTitleLine(0)}
                />
                {extraTitleLines.map((line, idx) => (
                  <React.Fragment key={line.id}>
                    <div className="group relative flex items-center gap-1">
                      {editable && (
                        <MoveBtn
                          dir="v"
                          onFirst={idx === 0}
                          onLast={idx === extraTitleLines.length - 1}
                          onNext={() => moveTitleLine(idx, 1)}
                          onPrev={() => moveTitleLine(idx, -1)}
                        />
                      )}
                      {mk(`tl-${line.id}`, line.text, v => updateTitleLine(line.id, v), {
                        placeholder: 'Custom line...'
                      })}
                      {editable && (
                        <div className="ml-auto flex-shrink-0 pl-4 opacity-0 transition-opacity group-hover:opacity-100">
                          <Tooltip title="Remove line">
                            <button
                              className="cursor-pointer border-0 bg-transparent p-1 text-red-300 hover:text-red-600"
                    ...
                    <TitleInsertGap
                      tooltip="Insert line after"
                      onInsert={() => insertTitleLine(idx + 1)}
                    />
                  </React.Fragment>
                ))}
              </>
    ...
    const renderFootnoteSection = (cssClass: string) => {
      ...
      return (
        <div
          className="mt-2 border-t pt-2 text-gray-500"
          onClick={e => e.stopPropagation()}
        >
          <style>{`
      ...
      `}</style>
          ...
          >
            <FootnoteInsertGap
              tooltip="Insert footnote at top"
              onInsert={() => insertFootnote(0)}
            />
            {displayNotes.map((fn: string, i: number) => (
              <React.Fragment key={i}>
                <div className="group relative flex items-start gap-1">
                  {editable && (
                    <MoveBtn
                      dir="v"
                      onFirst={i === 0}
                      onLast={i === displayNotes.length - 1}
                      onNext={() => moveFootnote(i, 1)}
                      onPrev={() => moveFootnote(i, -1)}
                    />
                  )}
                  <span className="mt-1.5 flex-shrink-0 text-10px text-gray-400">{i + 1}.</span>
                  {mk(`fn-${i}`, fn, v => onFnChange(i, v), { multiline: true, placeholder: 'Footnote...' })}
                  {editable && (
                    <Tooltip title="Remove footnote">
                      <button
                        className="ml-1 mt-1 flex-shrink-0 cursor-pointer border-0 bg-transparent text-red-400 opacity-0 transition-opacity hover:text-red-600 group-hover:opacity-100"
                ...
                <FootnoteInsertGap
                  tooltip={`Insert footnote after #${i + 1}`}
                  onInsert={() => insertFootnote(i + 1)}
                />
              </React.Fragment>
            ))}
            {displayNotes.length === 0 && (
              <div className="py-2 text-center text-xs text-gray-400">No footnotes — hover to add</div>
            )}
          </div>
        </div>
      );
    ...
    if (template.type === 'table') {
            ...
            >
              ...
              {level === 0 &&
                lblColumns.map((lbl, lblIdx) => (
                  <th
                    className="group relative px-2 py-1 text-left font-semibold"
                    key={lbl.id}
                    rowSpan={treeDepth}
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      {editable && lblColCount > 1 && (
                        <Tooltip title="Remove label column">
                          <button
                            className="cursor-pointer border-0 bg-transparent p-0 text-red-400 opacity-0 transition-opacity hover:text-red-600 group-hover:opacity-100"
                            style={{ fontSize: 10 }}
                            onClick={e => {
                              e.stopPropagation();
                              deleteLabelColumn(lbl.id);
                            }}
                          >
                            <DeleteOutlined />
                          </button>
                    ...
                    {editable && (
                      <div
                        className="absolute bottom-0 top-0 z-10 w-3 flex items-center justify-center opacity-0 transition-opacity -right-[6px] group-hover:opacity-100"
                        onMouseEnter={e => {
                          e.stopPropagation();
                        }}
                      >
                        <Tooltip title="Add label column">
                          <button
                            className="h-5 w-5 flex cursor-pointer items-center justify-center border-0 rounded-full bg-blue-500 text-xs text-white leading-none shadow-sm"
                            style={{ fontSize: 14, fontWeight: 300 }}
                            onMouseDown={e => e.stopPropagation()}
                            onClick={e => {
                              e.stopPropagation();
                              addLabelColumn(lbl.id);
                            }}
                          >
                            +
                          </button>
      ...
      const renderHeaderCell = (
        ...
        if (level === 0) {
          if (group.children?.length) {
            ...
            return (
              <th
                className="group relative px-2 py-1 text-center font-semibold"
                colSpan={span || leaves}
                key={group.id}
                rowSpan={rowSpan}
                style={colHeaderStyle}
          ...
          const useArmHeaders = Boolean(onColumnHeadersChange && columnHeaders);
          return (
            <th
              className="group relative px-2 py-1 text-center font-semibold"
              key={group.id}
                ...
                {editable && (
                  <Tooltip title="Delete column">
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        useArmHeaders ? deleteHeaderColumn(group.id) : delCol(group.id);
                      }}
                    >
                      <DeleteOutlined />
                    </button>
                ...
                >
                  <Tooltip title="Add column">
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        useArmHeaders ? addHeaderColumn(group.id) : addColumn();
                      }}
                    >
                      +
                    </button>
      ...
      return (
        <div
          className={`bg-white ${compact ? '' : 'p-3 border rounded'}`}
          onClick={() => setSel(null)}
        ...
        >
          ...
          `}</style>
            <table
              className="three-line-table w-full"
              style={{ fontSize: hs ? hs.columnHeaderSize : 11 }}
            >
              <thead>{buildHeaderRows()}</thead>
              <tbody>
                {rows.length === 0 ? (
                  <>
                    <RowInsertGap
                      tooltip="Insert first row"
                      onInsert={() => addRow(-1)}
                    />
                    <tr>
                      <td
                        className="px-2 py-4 text-center text-gray-400"
                        colSpan={totalHeaderCols}
                      >
                        No rows defined
                      </td>
                    </tr>
                  ...
                  <>
                    <RowInsertGap
                      tooltip="Insert row at top"
                      onInsert={() => addRow(-1)}
                    />
                    {rows.map((row, idx) => (
                      <React.Fragment key={row.id}>
                        <tr className="group transition-colors hover:bg-blue-50">
                          {/* Label columns cells */}
                          {lblColumns.map((lbl, lblIdx) => (
                            <td
                              className="px-2 py-1"
                              key={lbl.id}
                              style={{ paddingLeft: lblIdx === 0 ? (row.level || 0) * 20 + 8 : 8 }}
                            >
                              {lblIdx === 0 ? (
                                ...
                                <div className="flex items-center gap-2">
                                  {editable && (
                                    <MoveBtn
                                      dir="v"
                                      onFirst={prevSibling(rows, idx) < 0}
                                      onLast={nextSibling(rows, idx) < 0}
                                      onNext={() => moveRow(idx, 1)}
                                      onPrev={() => moveRow(idx, -1)}
                                    />
                                  )}
                                  {mk(`row-${row.id}`, row.label, v =>
                                    setShell(s => {
                                      s.rows = (s.rows || []).map((r: TableRow) =>
                                  ...
                                  {editable && (
                                    <>
                                      <Tooltip title="Outdent">
                                        <button
                                          className="cursor-pointer border-0 bg-transparent p-0 text-gray-400 opacity-0 transition-opacity hover:text-gray-600 group-hover:opacity-100"
                                          onClick={e => {
                                            e.stopPropagation();
                                            changeRowLevel(row.id, -1);
                                          }}
                                        >
                          ...
                          {(() => {
                            ...
                            const placeholder = hasStats
                              ? formatPlaceholder(
                                  ...
                                  buildDecimalsMap(
                                    row.stats!,
                                    decimalConfig?.shellDefaults,
                                    decimalConfig?.studyDefaults,
                                    DEFAULT_DECIMAL_RULES
                                  )
                                )
                              : null;
                        ...
                        <RowInsertGap
                          tooltip={`Insert row after "${row.label.slice(0, 20)}"`}
                          onInsert={() => addRow(idx)}
                        />
                      </React.Fragment>
                    ))}
                ...
                {Array.from({ length: maxHeaderLines }).map((_, lineIdx) => (
                  ...
                  >
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {isFirstLine && editable && (
                              <MoveBtn
                                dir="h"
                                onFirst={idx === 0}
                                onLast={idx === cols.length - 1}
                                onNext={() => moveCol(idx, 1)}
                                onPrev={() => moveCol(idx, -1)}
                              />
                            )}
                            ...
                            {editable ? (
                              <div className="flex flex-1 items-center gap-1">
                                {mk(
                                  `col-${col.id}-line-${lineIdx}`,
                                  lineText,
                                  v => updateHeaderLine(col.id, lineIdx, v),
                                ...
                                {editable && isFirstLine && (
                                  <Tooltip title="Add header line">
                                    <button
                                      className="cursor-pointer border-0 bg-transparent p-0 text-blue-400 opacity-0 transition-opacity hover:text-blue-600 group-hover:opacity-100"
                                      style={{ fontSize: 10 }}
                                      onClick={e => {
                                        e.stopPropagation();
                                        addHeaderLine(col.id);
                                      }}
                                    >
                                      +
                                    </button>
                                ...
                                {editable && lines.length > 1 && (
                                  <Tooltip title="Remove this line">
                                    <button
                                      className="cursor-pointer border-0 bg-transparent p-0 text-gray-400 opacity-0 transition-opacity hover:text-red-600 group-hover:opacity-100"
                                      style={{ fontSize: 10 }}
                                      onClick={e => {
                                        e.stopPropagation();
                                        removeHeaderLine(col.id, lineIdx);
                                      }}
                                    >
                                      ×
                                    </button>
                            ...
                            {isFirstLine && editable && (
                              <Tooltip title="Delete column">
                                <button
                                  className="ml-1 cursor-pointer border-0 bg-transparent text-red-400 opacity-0 transition-opacity hover:text-red-600 group-hover:opacity-100"
                                  style={{ fontSize: 10 }}
                                  onClick={e => {
                                    e.stopPropagation();
                                    delCol(col.id);
                    ...
                    {lineIdx === 0 && editable && cols.length === 0 && (
                      <th className="px-2 py-1 text-center text-gray-400">No columns — click + to add</th>
                    )}
                    {/* Final add column at the end - only on first line */}
                    {lineIdx === 0 && editable && cols.length > 0 && cols.length <= 10 && (
                      <th className="group-col-gap relative w-[32px] px-2 py-1">
                        <Tooltip title="Add column at end">
                          <button
                            className="h-5 w-5 flex cursor-pointer items-center justify-center border-0 rounded-full bg-blue-500 text-xs text-white leading-none opacity-60 shadow-sm transition-opacity hover:opacity-100"
                            style={{ fontSize: 14, fontWeight: 300 }}
                            onMouseDown={e => e.stopPropagation()}
                            onClick={e => {
                              e.stopPropagation();
    ...
    if (template.type === 'figure') {
      ...
      return (
        ...
        >
          ...
          <div
            className="min-h-[180px] flex flex-col items-center justify-center border border-gray-300 rounded border-dashed bg-gray-50 p-8"
            onClick={e => e.stopPropagation()}
          >
            <div className="mb-2 text-4xl">📊</div>
            <div className="text-base text-gray-600 font-medium">{fig.chartType || 'line'} Chart</div>
            <div className="mt-2 text-xs text-gray-400">
              X: {fig.xAxis?.label || '-'} | Y: {fig.yAxis?.label || '-'}
            </div>
            <div className="mt-1 text-xs text-gray-400">{fig.series?.length || 0} series</div>
          </div>
